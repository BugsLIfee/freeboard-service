...
public class JwtTokenProvider { // JWT 토큰을 생성 및 검증 모듈
    ...
    public String getName(String token) {
        return Jwts.parser().setSigningKey(secretKey.getBytes()).parseClaimsJws(token).getBody().get("name",String.class);
    }

    // Jwt 토큰에서 회원 구별 정보 추출
    public String getId(String token) {
        return Jwts.parser().setSigningKey(secretKey.getBytes()).parseClaimsJws(token).getBody().get("id", String.class);
    }
